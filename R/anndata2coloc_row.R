...
anndata2coloc_row <- function(coloc_combo_row, ad_by_chr, ad) {
  ...
  loc_start <- min(ad_obs[coloc_combo_row$t1,"start"], ad_obs[coloc_combo_row$t2,"start"])
  loc_start <- loc_start - 1e5

  loc_end <- max(ad_obs[coloc_combo_row$t1,"end"], ad_obs[coloc_combo_row$t2,"end"])
  loc_end <- loc_end + 1e5

  # Load-in precomputed lABF
  snps_for_coloc <- rownames(ad_var)[ad_var$pos >= loc_start & ad_var$pos <= loc_end]

  dataset <- ad_sm[c(coloc_combo_row$t1,coloc_combo_row$t2), snps_for_coloc]$X

  dataset1 <- dataset[coloc_combo_row$t1,]
  names(dataset1) <- snps_for_coloc
  #dataset1 <- dataset1[which(dataset1 != 0)]

  dataset2 <- dataset[coloc_combo_row$t2,]
  ...
  dataset1 <- impute.labf(
    ...
    imputed.labf = ad_obs[coloc_combo_row$t1,"min_res_labf"]
  )
  ...
  dataset2 <- impute.labf(
    ...
    imputed.labf = ad_obs[coloc_combo_row$t2,"min_res_labf"]
  )
  ...
  cojo_snp1 <- gsub(
    paste0(t1, "_(.*)_locus_.*_finemap.rds"),
    "\\1",
    coloc_combo_row$t1
  )
  ...
  cojo_snp2 <- gsub(
    paste0(t2, "_(.*)_locus_.*_finemap.rds"),
    "\\1",
    coloc_combo_row$t2
  )
